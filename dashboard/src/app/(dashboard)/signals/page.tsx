"use client";
import { Header } from "@/components/layout/Header";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { useSignals, useScreening } from "@/hooks/useAriaData";
import { fmt, timeAgo } from "@/lib/api";
import { cn } from "@/lib/utils";
import { Radio, TrendingUp, TrendingDown } from "lucide-react";

function ConfidenceBar({ value }: { value: number }) {
  const color = value >= 70 ? "bg-profit" : value >= 55 ? "bg-warning" : "bg-loss";
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 rounded-full bg-secondary overflow-hidden">
        <div className={cn("h-full rounded-full transition-all", color)} style={{ width: `${value}%` }} />
      </div>
      <span className="text-[11px] font-mono tabular-nums font-semibold w-6 text-right">{value}</span>
    </div>
  );
}

export default function SignalsPage() {
  const { data: signals, isLoading } = useSignals();
  const { data: biases } = useScreening();

  return (
    <div className="flex flex-col h-full">
      <Header title="Signal Feed" />
      <div className="flex-1 overflow-y-auto p-4 md:p-5 space-y-4">

        {/* Screening biases */}
        {biases && biases.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {biases.map((b) => (
              <Card key={b.symbol}>
                <CardContent className="p-3 flex items-center justify-between">
                  <div>
                    <p className="text-[14px] font-bold">{b.symbol}</p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">{timeAgo(b.ts)}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1.5">
                    <Badge variant={b.bias === "bullish" ? "profit" : b.bias === "bearish" ? "loss" : "muted"}>
                      {b.bias}
                    </Badge>
                    <div className="flex gap-1">
                      {b.allows_long  && <Badge variant="profit"   className="text-[9px] px-1.5 py-0.5">L</Badge>}
                      {b.allows_short && <Badge variant="loss"     className="text-[9px] px-1.5 py-0.5">S</Badge>}
                    </div>
                  </div>
                </CardContent>
              </Card>
            ))}
          </div>
        )}

        {/* Signal list */}
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              <Radio className="h-3.5 w-3.5 text-muted-foreground" />
              <CardTitle>Recent Signals</CardTitle>
              {signals && signals.length > 0 && (
                <span className="text-[10px] text-muted-foreground ml-auto">{signals.length} signals</span>
              )}
            </div>
          </CardHeader>
          <CardContent className="p-0">
            {isLoading && (
              <div className="flex items-center justify-center py-12 text-sm text-muted-foreground">Loading…</div>
            )}
            {!isLoading && (!signals || signals.length === 0) && (
              <div className="flex flex-col items-center justify-center py-12 gap-3">
                <div className="h-12 w-12 rounded-2xl bg-secondary flex items-center justify-center">
                  <Radio className="h-6 w-6 text-muted-foreground/50" />
                </div>
                <p className="text-sm text-muted-foreground">No signals yet</p>
              </div>
            )}
            <div className="divide-y divide-border/40">
              {signals?.map((s) => (
                <div key={s.signal_id} className="px-4 py-3.5 hover:bg-secondary/30 transition-colors">
                  <div className="flex items-center gap-2 flex-wrap">
                    {s.side === "LONG"
                      ? <TrendingUp className="h-4 w-4 text-profit shrink-0" />
                      : <TrendingDown className="h-4 w-4 text-loss shrink-0" />}
                    <Badge variant={s.side === "LONG" ? "profit" : "loss"}>{s.side}</Badge>
                    <span className="font-bold text-[14px]">{s.symbol}</span>
                    <Badge variant="secondary">{s.strategy.replace(/_/g, " ")}</Badge>
                    <Badge variant="muted" className="hidden sm:inline-flex">{s.regime}</Badge>
                    <span className="ml-auto text-[11px] text-muted-foreground whitespace-nowrap">{timeAgo(s.ts)}</span>
                  </div>

                  <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {[
                      { label: "Entry",       value: fmt(s.entry),      color: "" },
                      { label: "Stop Loss",   value: fmt(s.stop_loss),  color: "text-loss" },
                      { label: "Take Profit", value: fmt(s.take_profit),color: "text-profit" },
                    ].map(({ label, value, color }) => (
                      <div key={label}>
                        <p className={cn("text-[10px] mb-0.5", color || "text-muted-foreground")}>{label}</p>
                        <p className={cn("text-[12px] font-mono tabular-nums font-semibold", color || "text-foreground")}>{value}</p>
                      </div>
                    ))}
                    <div>
                      <p className="text-[10px] text-muted-foreground mb-1">TA Conf</p>
                      <ConfidenceBar value={s.ta_confidence} />
                    </div>
                  </div>

                  {s.reason && (
                    <p className="mt-2 text-[11px] text-muted-foreground leading-relaxed line-clamp-2">
                      {s.reason}
                    </p>
                  )}
                </div>
              ))}
            </div>
          </CardContent>
        </Card>
      </div>
    </div>
  );
}
